...
const rx_delete_default: RegExp = /[<>&%"\\]/g;
const rx_syntactic_variable: RegExp = /\{([^{}:\s]+)(?::([^{}:\s]+))?\}/g;
...
// 移除所有尖括号等字符
function default_encoder(replacement: string): string {
    return String(replacement).replace(rx_delete_default, "");
}

export default Object.freeze(function fulfill(
    string: string, // 含占位符的字符串
    container: any, // 含替换值的对象或数组, 函数
    encoder: any = default_encoder // 可选的编码函数或者包含多个编码函数的对象
) {
    return string.replace(
        rx_syntactic_variable,
        function (original: string, path: string, encoding: string = "") {
            try {
                // container
                ...
                let containerReplacement = typeof container === 'function'
                    ? container
                    : path.split(".").reduce(
                        function (acc: any, element: string) {
                            return acc[element];
                        },
                        container
                    );
